# Casandra — Open board (pick up here)

**Repo:** https://github.com/Kenyi001/oraculo-radiohead  
**Deadline:** Sun 23 Aug ~11:00 America/La_Paz  
**Tracks:** **General** (default) + sponsor **WDK**  
**Objetivo + roles:** [IDEA_CLARA.md](IDEA_CLARA.md) · [TEAM_BRIEF.md](TEAM_BRIEF.md) · [TRACK.md](TRACK.md) · [WDK.md](WDK.md) · [SHIP.md](SHIP.md)

## Objetivo (claro)

Agente IA → Casandra (riesgo + `action`) → solo si no es `avoid` → WDK USD₮. Demo + video + submit.

## Quién avanza qué

| Persona | GitHub | Issue | Hacer ahora |
|---------|--------|-------|-------------|
| **Victor** | @Vctor11180 | [#10](https://github.com/Kenyi001/oraculo-radiohead/issues/10) | Deploy Base Sepolia + address en README/demo |
| **David** | @arnez69 | [#17](https://github.com/Kenyi001/oraculo-radiohead/issues/17) | Implementar `get_market_pulse` (código, no más docs) |
| **Augusto** | @RonaldGaymer2002 | [#5](https://github.com/Kenyi001/oraculo-radiohead/issues/5) · [#6](https://github.com/Kenyi001/oraculo-radiohead/issues/6) | **Pitch + video + Vercel** — guía [PITCH_AUGUSTO.md](PITCH_AUGUSTO.md) |
| **Dax** | @Kenyi001 | [#8](https://github.com/Kenyi001/oraculo-radiohead/issues/8) · [#7](https://github.com/Kenyi001/oraculo-radiohead/issues/7) | Wallet WDK lista para el demo de Augusto + submit General+WDK |

## Orden (no bloquearse)

```
#10 Victor + #5 Augusto + #8 Dax  →  #6 video  →  #7 submit
#17 David en paralelo
```

## Ramas

| Rama | Estado |
|------|--------|
| `master` | Fuente de verdad (usar esta) |
| `rama-victor` | Mergeado; solo falta deploy #10 |
| `feature/Arnez` | Investigación mergeada; David → código #17 |
| `rama-augusto` | Vacía; Augusto toma #5/#6 desde master |

## Already done

#1 Specs · #2 core · #3 MCP · #4 UI · #9 Requirements · #13 risk_pct/verdict (cerrado)

## Full checklist

[REQUIREMENTS.md](REQUIREMENTS.md) · [TASKS.md](../TASKS.md) · [TEAM_BRIEF.md](TEAM_BRIEF.md)

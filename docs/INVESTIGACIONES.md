# Resumen de las investigaciones — Casandra

Síntesis de todo lo revisado sobre el proyecto **oraculo-radiohead / Casandra** (Aleph Hackathon 2026).

---

## 1. Estado del proyecto

- **Monorepo** TypeScript: `market-core` (lógica) → `mcp-server` (agentes IA) → `demo-web` (Vite + React).
- Rebrand a **Casandra — Investment Oracle MCP** (Aleph Hackathon 2026).
- Datos en vivo vía **CoinGecko** (+ mock si falla).
- Demo web: `npm run dev:web` → http://localhost:5173 (o 5174 si 5173 está ocupado).
- Repo: https://github.com/Kenyi001/oraculo-radiohead

```
packages/
  market-core/   # fetch + normalize + risk (shared)
...
## 2. Riesgo: porcentaje claro + veredicto legible

### Qué hay hoy

- Score **0–100** (`casandra-risk-v1`) + banda `low` / `med` / `high`.
- Factores explicables (`abs_change_24h`, volatilidad vs BTC, % USDT).
- Tool MCP `get_risk_level` devuelve JSON estructurado.
